...
#### Documentation
- **Team-to-Team Messaging** - Page-level changelog at the top of the page, listing per IG version what changed on this page (0.6.0 through 0.8.0), with a link to the full project changelog.
...
#### Profiles
- **OZOCareTeam** - Added `OZOPatient` to allowed types on `participant.member`. Self-reliant patients ("zelfredzame cliënten") can now participate in their own care network directly as Patient, without needing a separate RelatedPerson record.
...
#### Examples
- **Subscription examples** - Changed `status` from `requested` to `off` to prevent HAPI from activating example subscriptions with unreachable endpoints on server restart. Changed endpoint URLs from `example.nl` to `example.com`.

#### Build
- **Minimal package** - Example resources (Patient, Communication, Task, etc.) are now removed from the minimal package. Only conformance resources (profiles, ValueSets, CapabilityStatements) are kept. Prevents example data from being installed on production HAPI FHIR servers.
...
#### Examples
- All 9 Practitioner examples: identifier system updated to `ozoverbindzorg.nl/namingsystem/professional`
- All 4 Organization examples: identifier system updated to `ozoverbindzorg.nl/namingsystem/organization`
